...
`Advised resolution: none`, and no conflicting binding user-preference override, run this
...
- assume `User preference:` is absent, `none`, or otherwise non-binding on this path; if a
  conflicting binding user-preference override appears later—whether written as `Option <N>`,
  `<N>`, `one`, or with extra adjustment text—stop for clarification instead of silently
  normalizing it away
