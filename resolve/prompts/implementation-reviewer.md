...
- If the issue packet contains binding `User preference:` selecting `Option <N>` or `option <N>`,
  treat that option number as mandatory. Do not accept implementation that materially follows a
  different option or `Custom`; `Option <N>`, `Option <N> (recommended)`, and
  `Option <N> with slight refinement` are allowed only when they still implement the same option
  number and intent.
